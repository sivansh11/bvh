...
int main()
{
    ...
    std::vector<bvh::BVHInstance<Triangle>> instances;
    instances.push_back(instance);  // there can be multiple instances

    bvh::TLAS<Triangle> tlas;
    tlas.BVH_builder(instances);

    bvh::Ray ray{{0, 0, 0}, {0, 0, 1}};  // first origin then direction

    HitRecord hitRecord;
    if (tlas.intersect(ray, hitRecord))   // instead of intersecting with the bvh, or instance, we intersect with tlas
    {
